...
trading_days_2023 = nyse.schedule(start_date=start_date, end_date=end_date)
dates_only = pd.DataFrame(trading_days_2023.index.date)


# print(dates_only)
# print(dates_only.iloc[1, 0])
# print(type(dates_only))
# exit(0)

def compute_SMA(num_periods):
    stock_data['SMA', num_periods] = stock_data['Close'].rolling(
...
# this works --> print(stock_data['SMA', 20], stock_data['SMA', 50])
...
# loss criterion
criterion = nn.MSELoss() # nn.BCELoss() <- BCE is better if the dataset is larger in this case
...
def create_labels(days_after_start_record):
    ...
    for i, value in part_of_data.iterrows():
        ...
        x[count] = torch.tensor([day_data['Open'], day_data['Adj Close'],
                                 day_data['SMA', 20], day_data['SMA', 50],
                                 day_data['SMA', 100],
